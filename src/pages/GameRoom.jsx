...
export default function GameRoom({ roomCode, playerName, onLeave }) {
  ...
  const myColor = AVATAR_COLORS[(myPlayer?.color_index || 0) % AVATAR_COLORS.length]
      ...
      <div className="app" style={{ maxWidth: 480, margin: '0 auto', width: '100%' }}>
        <div className="fade-in" style={{ width: '100%', paddingTop: '1.5rem' }}>
          <div className="text-center mb-lg">
            ...
            <div className="room-code" onClick={copyCode} style={{ cursor: 'pointer' }} title="Cliquer pour copier">
              {roomCode}
            </div>
          ...
          <div className="card mb-md">
            ...
            {totalPlayers >= 2 && (
              <div className="card mt-md" style={{ background: 'var(--bg3)', padding: '0.75rem 1rem', fontSize: 12, color: 'var(--text2)' }}>
                ...
                Élimination : <strong style={{ color: 'var(--danger)' }}>−30 pts</strong>
              </div>
            )}
          </div>
          ...
          {isHost ? (
            ...
            <div className="card text-center" style={{ color: 'var(--text2)', fontSize: 14 }}>
              <span className="pulse">En attente que l'hôte lance la partie…</span>
            </div>
          )}
          ...
          <div className="card mb-md">
            {alreadyChosen ? (
              <div className="text-center" style={{ padding: '1rem 0' }}>
                ...
                <p className="text-muted text-sm mt-sm pulse">
                  En attente de {waitingFor} joueur{waitingFor > 1 ? 's' : ''}…
                </p>
                {isHost && allChosen && (
                  <button className="btn btn-success mt-md" onClick={revealAndScore}>
                    Révéler les choix →
                  </button>
                )}
              </div>
              ...
              myPlayer?.eliminated ? (
                <div className="text-center" style={{ padding: '1rem 0' }}>
                  <p style={{ color: 'var(--warn)', fontFamily: 'Syne', fontWeight: 700 }}>Vous êtes éliminé</p>
            ...
            <div className="flex flex-col mt-sm" style={{ gap: 8 }}>
              {activePlayers.map(p => (
                <div key={p.id} className="flex items-center gap-sm">
                  ...
                  <span className="text-xs text-muted">{p.has_chosen ? 'Prêt ✓' : 'En train de choisir…'}</span>
                </div>
              ))}
            </div>
            {isHost && allChosen && (
              <button className="btn btn-success btn-full mt-md" onClick={revealAndScore}>
                Tous prêts — Révéler →
              </button>
            )}
          </div>
    ...
    return (
      <div className="app" style={{ maxWidth: 520, margin: '0 auto', width: '100%' }}>
        <div className="fade-in" style={{ width: '100%', paddingTop: '1rem' }}>
          ...
          {isHost ? (
            <button className="btn btn-primary btn-full" style={{ padding: 14 }} onClick={nextTurn}>
              Tour suivant →
            </button>
          ) : (
            <div className="card text-center text-muted text-sm">
              <span className="pulse">En attente de l'hôte…</span>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fade-in text-center" style={{ width: '100%', paddingTop: '2rem' }}>
          {winner ? (
            <div className="card mb-md" style={{
              ...
              <div className="avatar pop" style={{
                background: winColor.bg, color: winColor.text,
                margin: '0 auto 12px', width: 56, height: 56, fontSize: 20,
              }}>
